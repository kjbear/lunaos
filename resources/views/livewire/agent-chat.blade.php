<div class="flex h-screen bg-slate-950">
    ...
    <main class="flex-1 flex flex-col">
        @if ($selectedMember)
            ...
            <!-- Messages -->
            <div class="flex-1 overflow-y-auto px-6 py-4 space-y-4" id="messages-container" x-data x-ref="container" 
                x-init="$nextTick(() => $refs.container.scrollTop = $refs.container.scrollHeight)"
                x-on:livewire-update-messages.window="$nextTick(() => $refs.container.scrollTop = $refs.container.scrollHeight)">
                
                <!-- User message shown immediately via JS -->
                <template x-if="$wire.pendingUserMessage">
                    <div class="flex gap-3 justify-end">
                        <div class="max-w-2xl bg-purple-600 rounded-2xl px-4 py-3">
                            <p class="text-white text-sm whitespace-pre-wrap" x-text="$wire.pendingUserMessage"></p>
                            <p class="text-xs text-purple-200 mt-1">Just now</p>
                        </div>
                    ...
                    <div class="flex gap-3 {{ $message['role'] === 'user' ? 'justify-end' : 'justify-start' }}">
                        ...
                        <div class="max-w-2xl {{ $message['role'] === 'user' ? 'bg-purple-600' : 'bg-slate-800' }} rounded-2xl px-4 py-3">
                            ...
                            <div class="flex items-center justify-between mt-1">
                                <p class="text-xs {{ $message['role'] === 'user' ? 'text-purple-200' : 'text-slate-500' }}">
                                    {{ $message['timestamp'] }}
                                </p>
                    ...
                    <div class="flex gap-3 justify-start">
                        ...
                        <div class="max-w-2xl bg-slate-800 rounded-2xl px-4 py-3">
                            <p class="text-white text-sm whitespace-pre-wrap">
                                <span wire:stream="stream-response">{{ $streamingResponse }}</span>
                            </p>
                        </div>
                    </div>
                @endif
            ...
            <div class="border-t border-slate-800 bg-slate-900 p-4">
                <form wire:submit="sendMessage" class="flex gap-3" x-data x-on:submit="setTimeout(() => $wire.pendingUserMessage = '', 50)">
                    <input
                        type="text"
